...
export const seeActiveTodos = () => async (dispatch) => {
  const url =
    "https://652f72f90b8d8ddac0b27cb8.mockapi.io/todo?isFinished=false";
  const { data } = await axios(url);

  // console.log(data)
  dispatch(successGetTodo(data));
};
export const seeCompletedTodos = () => async (dispatch) => {
  const url =
    "https://652f72f90b8d8ddac0b27cb8.mockapi.io/todo?isFinished=true";
  const { data } = await axios(url);
